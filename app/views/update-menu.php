      ...
      <div class="min-h-[75vh] flex items-center">
        <div class="w-full">
          ...
          <form id="update-menu-form" autocomplete="off">
            <input type="hidden" name="menu" value="<?php echo $menu_info->menu_id ?>">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-10">
              <div class="file-upload grid place-items-center bg-gray-100 h-[16rem] md:h-auto relative cursor-pointer rounded-2xl">
                ...
                <img src="<?php echo SYSTEM_URL.'/uploads/menu/'.$menu_info->menu_id.'.png' ?>" alt="menu image" class="preview-img md:w-full max-h-[16rem] md:max-h-[23rem] object-contain">
              ...
              <div>
                <div class="mb-3">
                  ...
                  <div class="flex h-14 border border-slate-400/60 rounded-full py-2 px-8 mb-4 sm:mb-0">
                    <input type="text" placeholder="Input the menu name" name="menu_name" class="flex-1 text-xs text-slate-400 placeholder:text-slate-400 outline-none" value="<?php echo $menu_info->menu_name ?>">
                  </div>
                </div>
                <div class="grid md:grid-cols-1 lg:grid-cols-2 gap-4 mb-3">
                  <div>
                    ...
                    <div class="flex h-14 border border-slate-400/60 rounded-full py-2 px-8 mb-4 sm:mb-0">
                      <input type="text" placeholder="Input the menu price" name="menu_price" class="numeric flex-1 text-xs text-slate-400 placeholder:text-slate-400 outline-none" value="<?php echo $menu_info->menu_price ?>">
                    </div>
                  </div>
                  <div>
                    ...
                    <div class="flex h-14 border border-slate-400/60 rounded-full py-2 px-8 mb-4 sm:mb-0">
                      <input type="text" placeholder="Input the menu price" name="stock" class="numeric flex-1 text-xs text-slate-400 placeholder:text-slate-400 outline-none" value="<?php echo $menu_info->menu_stock ?>">
                    </div>
                  </div>
                </div>
                ...
                <div class="custom-select relative flex items-center justify-between h-14 border border-slate-400/60 rounded-full py-2 px-8 mb-4 sm:mb-4 cursor-pointer">
                  <input type="hidden" class="category-input" name="category" value="<?php echo $menu_info->category_id ?>">
                  ...
                  <?php foreach($category->get() as $type): ?>
                    <li class="custom-select-option flex items-center text-[10px] md:text-xs text-slate-500 font-medium gap-3 py-2 hover:bg-gray-100 px-6 rounded-2xl transition-all <?php echo $type->category_id == $menu_info->category_id ? 'selected' : '' ?>" data-category="<?= $type->category_id ?>">
                ...
                <input type="hidden" class="tag-input" name="menu_tag" value="<?php echo $menu_info->menu_tag ?>">
                <div class="flex flex-wrap gap-3 mb-8">
                  <div class="tag-option <?php echo $menu_info->menu_tag == 'New' ? 'selected' : '' ?>">
                    New
                  </div>
                  <div class="tag-option <?php echo $menu_info->menu_tag == 'Not Available' ? 'selected' : '' ?>">
                    Not Available
                  </div>
                </div>
                <div class="flex">
